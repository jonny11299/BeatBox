//much like NoteSquare except it's a square
...
class NoteSquare{
  ...
  NoteSquare(Oscillator oscIn, float freq, float volume, SmartEnv envIn, float xIn, float yIn){
    osc = oscIn;
    env = envIn;
  ...
  NoteSquare(Oscillator oscIn, float freq, float volume, float pan, SmartEnv envIn, float xIn, float yIn, float sizeIn, Color cIn, Color bcIn){
    osc = oscIn;
    env = envIn;
