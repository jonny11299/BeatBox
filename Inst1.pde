class Inst1 extends Instrument{
  int beats = 6; //number of beats before this instrument loops
  ...
  Inst1(BeatBox soundBox){
    super(soundBox);
    baseFreq = soundBox.defaultPitch;
    myName = "inst1";
    screens.add(new Screen(soundBox, "inst1", 4, 58, 992, 690, HELPER.BLUE, 4, 3)); //border screen
    screens.add(new Screen(soundBox, "inst1", 8, 62, 1000 - 92, 76, HELPER.BLACK, 4, 3)); //beats slider screen on top
    screens.add(new Screen(soundBox, "inst1", 8, 142, 300, 750 - 148, HELPER.BLACK, 4, 3)); //ADSR sliders
    instScreen = new Screen(soundBox, "inst1", 312, 142, 680, 750 - 148, HELPER.BLACK, HELPER.BLACK, 4, 3); //THE screen
    ...
    sliders.add(new Slider(270, 100, 600, true, 6/16, new Color(255, 255, 30))); //beat length slider
    sliders.add(new Slider(38, 726, 520, false, 0.01)); //A
    sliders.add(new Slider(79, 726, 520, false, 0.2)); //D
  ...
  void addNote(int step, float freq, float vol, float size, float x, float y){          
    if (numNotes < MAX_NOTES){
      ...
      Color bodyColor = new Color(floor(map(abs(pan), 0, 1, 0, 255)),
                                  floor(random(256)), 
                                  floor(map(y, instScreen.y, instScreen.y + instScreen.h, 0, 255)));
      Color borderColor = HELPER.WHITE;
      
      instSteps[step].makeNote(new NoteDot(new TriOsc(APPLET), //DEFAULT OSCILLATOR TYPE: This one is set to TriOsc
  ...
  void addDefaultNotes(){
    ...
    addNote(1 * 8 + 1, x + w/2 + 200, y + h - 12*(h/24) - 10);
    addNote(1 * 8 + 5, x + w/2 + 120, y + h - 14*(h/24) - 10);
    addNote(2 * 8 + 1, x + w/2 + -210, y + h - 23*(h/24) - 10);
    addNote(3 * 8 + 1, x + w/2 + -170, y + h - 19*(h/24) - 10);
  }
}
